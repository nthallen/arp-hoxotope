...
JHNO3_FIT1=f_jsza(abs(sza_nloop),1.0e-6,.945,2.75);
JHNO3_FIT2=f_jsza(abs(sza_nloop),.75e-6,.875,0.775);

jho2no2=JHNO3_FIT1+JHNO3_FIT2;
...
% HO2NO2    
   Pho2no2 = kohno2n(T,M).*NO2.*OH*dt_nloop;
   Lho2no2 = (-OH.*HO2NO2.*kohhno3n(T,M)-jho2no2.*HO2NO2).*dt_nloop;
...
% NO 
   ...
   OH_in=jho2no2./kohhno3n(T,M);
   rat=ho2oh(T,O3,NO,0,1e7,5e6,50);
   HO2_in = rat.*OH;
 
if HO2_in < 0; HO2_in=0; OH_in=0; end
...
if Jho2no2_out(k)==0; HO2NO2ss(k)=0; else
   HO2NO2ss(k)=kohno2n(T,M).*NO2_out(k).*OH_out(k)./(Jho2no2_out(k)+OH_out(k).*kohhno3n(T,M));
end; end;
