...
for d=1:length(month)
   sza=szac1(lat_97(d),long_97(d),0:36000:86400,month(d),day_of_month(d),1997);
   j=find(sza<94);
   if length(j)>0
...
day_of_month=day_of_year-(month-1).*30;
for d=1:length(month)
   sza=szac1(lat_94(d),long_94(d),0:36000:86400,month(d),day_of_month(d),1994);
   j=find(sza<94);
   if length(j)>0
      avg_sza(d)=median(abs(sza(j)));
      fakeOH(d)=median(2.7e4.*(100-abs(sza(j))));
...
save lod_ju lod* avg_oh_* avg_sza_* avg_j*
