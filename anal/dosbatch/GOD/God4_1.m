...
% XX  version 4.1 Modified after POLARIS to use 100 torr        XX
% XX  baratron and to correct baseline effects during the 	    XX
% XX  sunrise/sunset flights. This is the final archive version.XX
% XX  Modified 12/3/99 for SOLVE directories							 XX
% XX New 1000 torr baratronhas new zero 								 XX
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
%pause
...
% During POLARIS <SD__P - SD_P2> = 12.1 +/- 1.1 torr.  SD_P2_0 = 0.4 torr
% If SD_P2 is correct, <sd__p0> = 11.7 and ranges from 10.4 to 13.7 torr.
% Use 100 torr baratron to zero 1000 torr.  Use 1000 tor for all P's.
% For now, use sd_p2_0=1.0 for the 100 torr baratron zero.
% SOLVE test flights sd_p0=16.8, sd_p2_0=2.0
load \solve\batch\zero
sd_p2_0=2.0;
i=find(sd__p_hox < 100);
sd__p0=median(sd__p_hox(i) - (sd_p2_hox(i)-sd_p2_0));
%
...
i=find(sd_p2_hox<110);
press_hox(i)=sd_p2_hox(i)-sd_p2_0;
i=find(sd_p2_oh<110);
press_oh(i)=sd_p2_oh(i)-sd_p2_0;
i=find(sd_p2_n2<110);
press_n2(i)=sd_p2_n2(i)-sd_p2_0;
i=find(sd_p2_no<110);
press_ho2(i)=sd_p2_no(i)-sd_p2_0;
i=find(sd_p2_nob<110);
press_ho2b(i)=sd_p2_nob(i)-sd_p2_0;

% Correct pressure for leak on 970626.
if fltday=='970626'
   press_hox=press_hox-7.2;
   press_oh=press_oh-7.2;
   press_n2=press_n2-7.2;
   press_ho2=press_ho2-7.2;
   press_ho2b=press_ho2b-7.2;
end
...
dx1sig_hoxf=dx1sig_hox+sx1sig_hox-sx1sig_hoxf;
   if day==9981  % On this sunrise flight,we fit the baseline of each
      % scan and linear interpolate the smoothed result.  The positive
      % offset for the first point is reproducible in the fits. Use the median
      % for the last data points at max(time_hox);
      time_scan=[36950 39920 42805 45700 48600 51460 54340 57230 max(time_hox)]';
      offset1=[0.5 -1.8 -1.8 -2.8 -1.1 -1.3 -2.9 -2.2 -1.8]';
      offset2=[0.7 -1.1 -0.6 -1.8 -2.7 -1.5 -1.0 -1.1 -1.0]';
      fit1=interp1(time_scan,nmedianf(offset1,1),time_hox);
      fit2=interp1(time_scan,nmedianf(offset2,1),time_hox);
      dx1sig_hoxf=dx1sig_hoxf+fit1;
      dx2sig_hox=dx2sig_hox+fit2;
   end
...
if day==10115  %on this sunset flight, the scatter changed by 
		% approximately 1 and 0.5 sturdley from the offline 
      % to the online position after 1.65e4 s, midway between scans 4 & 5.
      i=find(time_hox>1.65e4);
      dx1sig_hoxf(i)=dx1sig_hoxf(i)-1;
      dx2sig_hox(i)=dx2sig_hox(i)-0.5;
end
...
i=find(isnan(temp_hox));
temp_hox(i)=temp_hox(i-1);
press_hox(i)=press_hox(i-1);
...
% Set diff_hoxf equal to 1.3 when HO2 titration signal is small
j=find(topsig2<20);
diff_hoxf(j)=1.3.*diff_hoxf(j)./diff_hoxf(j);
j=find(topsig2b<20);
diff_hoxfb(j)=1.3.*diff_hoxfb(j)./diff_hoxfb(j);
% 970626 use weaker filter to avoid washing out fast velocity changes
% Otherwise, filter diff_hox to get smooth correction
i=find(isnan(diff_hox)==0);
if day~=10038
   diff_hoxf(i)=nmedianf(diff_hoxf(i)',5);
   diff_hoxfb(i)=nmedianf(diff_hoxfb(i)',5);
end
%
